...
```bash
step-functions-sample$ sam build
```
...
```bash
step-functions-sample$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
step-functions-sample$ sam local start-api
step-functions-sample$ curl http://localhost:3000/
```
...
```bash
step-functions-sample$ sam logs -n HelloWorldFunction --stack-name step-functions-sample --tail
```
...
```bash
step-functions-sample$ cd hello-world
...
```bash
sam delete --stack-name step-functions-sample
```
